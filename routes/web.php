...
Route::get('vvv', [VersionUpdateController::class, 'versionUpdate'])->name('version-update');
Route::post('ppp', [VersionUpdateController::class, 'processUpdate'])->name('process-update');
...
Route::get('test', [WalletRechargeController::class, 'test'])->name('test');

Route::controller(\App\Http\Controllers\CaptchaController::class)->group(function () {
    Route::get('/generate-captcha','generate_captcha')->name('captcha.generate');
    Route::get('/refresh-captcha',  'refreshCaptcha')->name('captcha.refresh');
});

Route::get('maps',function () {
    $response = \GoogleMaps::load('geocoding')
        ->setParam (['address' =>'pakistan'])
        ->get();
    return dd($response);
});
