...
Route::post('secure-login', [LoginController::class, 'secure_login'])->name('frontend.secure.login');
Route::post('secure-login-auth', [LoginController::class, 'secure_login_auth'])->name('frontend.secure.login.auth');
Route::post('secure-logout', [LoginController::class, 'secure_logout'])->name('frontend.secure.logout');
...
Route::get('generate-verification-page/{email}', [LoginController::class, 'generateVerificationPage'])->name('generate-verification-page');
Route::post('send-verification-code', [LoginController::class, 'sendVerificationCode'])->name('send-verification-code');
...
// verification
Route::get('generate-verification-page', [LoginController::class, 'showVerifyLogin'])->name('verify.login')->middleware('auth')->withoutMiddleware('guest');
Route::get('verify-login-code', [LoginController::class, 'showVerifyLoginCode'])->name('verify.login.code')->middleware('auth')->withoutMiddleware('guest');
Route::post('send-login-verification-code', [LoginController::class, 'sendLoginVerificationCode'])->name('verification.send')->middleware('auth')->withoutMiddleware('guest');
Route::post('verify-login', [LoginController::class, 'verifyLogin'])->name('verify.login.post')->withoutMiddleware('guest');
Route::post('refresh-captcha', function () {
    try {
        return response()->json(['status' => true, 'captcha' => captcha_img()]);
    } catch (Exception $e) {
        return response()->json(['status' => false, 'captcha' => $e->getMessage()]);
    }
...
Route::group(['middleware' => ['private.mode', 'verify.login']], function () {
    ...
    Route::get('{roles?}/profile/LS{user?}{type?}/{name?}', [MainIndexController::class, 'userProfile'])->name('userProfile');
    ...
    Route::get('organizations', [MainIndexController::class, 'organizations'])->name('instructor');
    Route::get('teachers', [MainIndexController::class, 'teachers'])->name('instructor');
    ...
    Route::get('events', [EventController::class, 'blogAll'])->name('events');

    Route::get('event-details/{slug}', [EventController::class, 'blogDetails'])->name('event-details');
