<div class="card instructor-item search-instructor-item position-relative text-center border-0 user-item {{ $type == INSTRUCTOR_CARD_TYPE_THREE ? 'p-20' : 'p-30' }} px-3" data-user-id="{{ $user->id }}">
    @php
    ...
    <a
        href="{{ route('userProfile', [
                'roles' => $user->is_organization() ? 'organizations' : 'teachers',
                'user' => $user->id,
                'type' => $user->is_organization() ? 'or' : 't',
                'name' => $user->is_organization() ? $user->organization->full_organization_name_slug : $user->instructor->full_instructor_name_slug
                ]) }}">
            <img src="{{ $user->getAvatarUrl($user->is_organization() ? 'organization' : 'teacher') }}" alt="instructor"
                class="fit-image rounded-circle">
      </a>
    </div>
    <div class="card-body p-0">
        <h6 class="card-title"><a href="{{ route('userProfile', [
                    'roles' => $user->is_organization() ? 'organizations' : 'teachers',
                    'user' => $user->id,
                    'type' => $user->role == USER_ROLE_ORGANIZATION ? 'or' : 't',
                    'name' => $user->is_organization() ? $user->organization->full_organization_name_slug : $user->instructor->full_instructor_name_slug
            ]) }}">{{
                $user->name }}</a>
        </h6>
        ...
        <div class="w-100">
            ...
            <a {{ $type == INSTRUCTOR_CARD_TYPE_THREE ? 'target=_blank' : '' }} href="{{ route('userProfile', [
                    'roles' => $user->is_organization() ? 'organizations' : 'teachers',
                    'user' => $user->id,
                    'type' => $user->is_organization() ? 'OR' : 'T',
                    'name' => $user->is_organization() ? $user->organization->full_organization_name_en : $user->instructor->slug
                ]) }}"
