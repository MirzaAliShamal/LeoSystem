<div class="tab-pane fade" id="Instructor" role="tabpanel" aria-labelledby="Instructor-tab">
    ...
    @if (file_exists(public_path($course_instructor->user->image_path)))
        <img src="{{ getImageFile($course_instructor->user->image_path) }}" alt="img">
    @else
        @php
            $userId = $course_instructor->user->id;
            $imagePath = "uploads/{$userId}/user/{$course_instructor->user->image}";
            $filePath = public_path($imagePath);
        @endphp
        @if (file_exists($filePath))
            <img src="{{ asset($imagePath) }}" alt="User Image" class="avater-imag">
        @else
            {{-- Fallback to a default image or provide an alternative --}}
            <img src="{{ asset('path_to_default_image') }}" alt="Default User Image" class="avater-imag">
        @endif
    @endif
</a>
                                </div>
