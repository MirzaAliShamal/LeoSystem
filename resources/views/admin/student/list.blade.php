                                ...
                                @foreach($students as $student)
                                    ...
                                    <tr class="removable-item">
                                        ...
                                        <td>
                                            ...
                                            @if ($student->user && $student->user->image_path)
                                                @php
                                                    $firstPath = public_path('uploads/' . $student->user->image_path);
                                                    $secondPath = $student->user->image_path;
                                                @endphp

                                                <!-- Show image from the first path -->
                                                <img src="{{ $student->user->getAvatarUrl('student') }}"
                                                     width="80" style="border-radius: 18px;" alt="img">
                                            @else
                                                <img src="{{ asset(REG_DEFAULT_STUDENT) }}"
                                                     width="80" style="border-radius: 18px;" alt="img">
                                            @endif
                                        ...
                                        <td>
                                            {!!html_entity_decode("<b>Student ID: </b>")!!}LS{{$student->user?->id}}ST
                                            <br>
                                            {{__('Name')}}: {{$student->name}}<br>

                                            {{__('Email')}}: {{$student->user?->email}}<br>

                                            {{__('Phone')}}: +({{$student->user?->area_code}}
                                        ...
                                        <td>
                                            ...
                                            <select name="status"
                                                ...
                                                <option value="1"
                                                        @if($student->status == 1) selected @endif>{{ __('Approved') }}</option>

                                                <option value="2"
                                                        @if($student->status == 2) selected @endif>{{ __('Blocked') }}</option>
                                        ...
                                        <td>
                                            ...
                                            <div class="action__buttons">
                                                ...
                                                <button class="ms-3">
                                                    ...
                                                    <span data-formid="delete_row_form_{{$student->id}}"
                                                          class="deleteItem">
                                                ...
                                                <form action="{{route('student.delete', [$student->uuid])}}"
                                                      method="post" id="delete_row_form_{{ $student->id }}">
