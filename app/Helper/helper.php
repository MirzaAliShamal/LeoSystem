...
function getDefaultAvatar()
{
    return match (\auth()->user()->role) {
        USER_ROLE_INSTRUCTOR => asset('uploads/default/reg-default-teacher.jpg'),
        USER_ROLE_ORGANIZATION => asset('uploads/default/reg-default-organization.jpg'),
        USER_ROLE_STUDENT => asset('uploads/default/reg-default-student.jpg'),
        default => assert('uploads/default/user.png'),
    };
}
...
if (!function_exists('isEnableOpenAI')) {
    ...
    {
        $role = auth()->user()?->role;
        return isAddonInstalled('LMS') && in_array($role, [USER_ROLE_INSTRUCTOR,USER_ROLE_ORGANIZATION, USER_ROLE_ADMIN]) && get_option('open_ai_system');
    }
}
