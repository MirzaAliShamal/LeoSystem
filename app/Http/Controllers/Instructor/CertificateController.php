...
class CertificateController extends Controller
{
    ...
    public function store(Request $request, $course_uuid, $certificate_uuid)
    {
        ...
        // $certificate_by_instructor->signature = $request->signature ? $this->saveImage(Auth::user()->id.'/certificate', $request->signature, '120', '60') :   null;
        $signature = $this->uploadFileWithDetails(Auth::user()->id.'/certificate', $request->signature);
        $certificate_by_instructor->signature = $signature['path'];
    ...
    public function update(Request $request, $uuid)
    {
        ...
        if ($request->signature)
        {
            ...
            // $certificate_by_instructor->signature =$this->saveImage(Auth::user()->id.'/certificate', $request->signature, '120', '60') ;
            $signature = $this->uploadFileWithDetails(Auth::user()->id.'/certificate', $request->signature);
            $certificate_by_instructor->signature = $signature['path'];
        }
        $certificate_by_instructor->save();
