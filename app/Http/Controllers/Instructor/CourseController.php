...
class CourseController extends Controller
{
    ...
    public function create()
    {
        ...
        // dd($countCourseInstructor, $countCourse, $count, $packageType);
    ...
    public function saveCourseImage(Request $request, $uuid): \Illuminate\Http\JsonResponse
    {
        ...
        try {
            ...
            if ($request->image_base64) {
                ...
                $this->deleteFile($course->catalogue_image);

                $request->validate([
                    'image' => 'mimes:jpg,png,jpeg,gif,svg'
                ]);

                $image = $this->saveImageFromBase64(Auth::user()->id . '/course', $request->image_base64, $request->width, $request->height); // new file upload into server

                // storing again for catalogue section (800 x 533)
                $catalogue_image = $this->saveImageFromBase64(Auth::user()->id . '/course', $request->image_base64, 800, 600, 'jpg', '-catalogue'); // new file upload into server

            } else {
                $image = $course->image;
                $catalogue_image = $course->catalogue_image;
            }
            $course->image = $image;
            $course->catalogue_image = $catalogue_image;
            $course->save();
    ...
    public function updateCategory(Request $request, $uuid)
    {
        ...
        if ($request->video) {
            ...
            $file_details = $this->uploadFileWithDetails(Auth::user()->id . '/course', $request->video);
        ...
        // Update the course if any data has changed
        if ($hasChanged) {
            $this->model->updateByUuid($data, $uuid);
            $course->status = 2;
            $course->save();
        ...
        $hasChangedTag = false;
        if ($request->tag) {
            $existingTags = $course->tags->pluck('id')->toArray();
            $newTags = $request->tag;
            sort($existingTags);
            sort($newTags);
    ...
    private function storeBase64($imageBase64, $folderName = 'course')
    {
