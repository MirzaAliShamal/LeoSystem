...
class StudentController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'gender' => 'required',
            'about_me' => 'required',
            'image' => 'mimes:jpeg,png,jpg|file|dimensions:min_width=700,min_height=700,max_width=5000,max_height=5000|max:5000'
        ]);
        ...
        $user->role = 3;
        // $user->image =  $request->image ? $this->saveImage(Auth::user()->id.'/user', $request->image, null, null) :   null;
//        $image =  $this->uploadFileWithDetails(Auth::user()->id.'/user', $request->image);
    ...
    public function update(Request $request, $uuid)
    {
        ...
        $user->mobile_number = $request->phone_number;
        $user->phone_number = $user->area_code . $request->phone_number;
        // $user->image =  $request->image ? $this->saveImage(Auth::user()->id.'/user', $request->image, null, null) :   $user->image;
//        $image =  $this->uploadFileWithDetails(Auth::user()->id.'/user', $request->image);
        if($request->image_base64){
            $image = $this->saveImageFromBase64(Auth::user()->id.'/user', $request->image_base64,700,700); // new file upload into server
            $user->image =  $image;
