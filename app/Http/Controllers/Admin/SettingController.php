...
class SettingController extends Controller
{
    ...
    public function GeneralSettingUpdate(Request $request)
    {
        ...
        // dd($request->hasFile('become_instructor_video'));
        ...
        foreach ($inputs as $key => $value) {
            ...
            } elseif ($request->hasFile('become_student_video_logo') && $key == 'become_student_video_logo') {

                $request->validate([

                    'become_student_video_logo' => 'mimes:png|dimensions:min_width=70,min_height=70,max_width=70,max_height=70'

                ]);

                $this->deleteFile(get_option('become_student_video_logo'));

                $option->option_value = $this->saveImage('setting', $request->become_student_video_logo, null, null);
            ...
            } elseif ($request->hasFile('become_student_video_preview_image') && $key == 'become_student_video_preview_image') {

                $request->validate([

                    'become_student_video_preview_image' => 'mimes:png|dimensions:min_width=835,min_height=630,max_width=835,max_height=630'

                ]);

                $this->deleteFile(get_option('become_student_video_preview_image'));

                $option->option_value = $this->saveImage('setting', $request->become_student_video_preview_image, null, null);

                $option->save();

            } elseif ($request->hasFile('become_student_video') && $key == 'become_student_video') {

                $this->deleteVideoFile(get_option('become_student_video'));



                $file_details = $this->uploadFileWithDetails('setting', $request->become_student_video);
