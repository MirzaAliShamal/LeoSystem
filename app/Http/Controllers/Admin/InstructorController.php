...

{
    ...
    protected $instructorModel, $studentModel, $blockingHinstoryModel;

    public function __construct(Instructor $instructor, Student $student, BlockingHistory $blockingHistory)

    {
        ...
        $this->blockingHinstoryModel = new Crud($blockingHistory);
...

    {
        ...
        $data['title'] = __('Teachers Blocking History');

        $data['blocked_teachers'] = BlockingHistory::orderBy('id', 'DESC')->teachers()->paginate(25);

        return view('admin.instructor.blocking_history', $data);
...

    {
        ...
        if (!Auth::user()->can('pending_teacher')) {
...

    {
        ...
        if (!Auth::user()->can('approved_teacher')) {
...

    {
        ...
        try {
            ...
            Mail::to($instructor->user->email)->send(new UserApprovedMail($instructor->user,'LS'.$instructor->user->id.'T','Teacher'));
...

    {
        ...
        if (!Auth::user()->can('manage_teacher')) {
        ...
        Mail::to($instructor->user->email)->send(new UserRejectedMail($instructor->user,$instructor->status_message,'Teacher'));
    ...
    public function changeInstructorStatus(Request $request): \Illuminate\Http\JsonResponse

    {
        try {

            $instructor = $this->instructorModel->getRecordById($request->id);
            $instructor->status = $request->status;
            $instructor->status_message = $request->status_message;

            switch ($instructor->status){
                case STATUS_APPROVED:
                    $instructor->save();
                    $this->blockingHinstoryModel->create([
                        'manage_user' => auth()->id(),
                        'user' => $instructor->user->id,
                        'message' => $request->input('status_message'),
                        'type'    => USER_ROLE_INSTRUCTOR,
                        'status'  => STATUS_APPROVED
                    ]);
                    Mail::to($instructor->user->email)->send(new UserApprovedMail($instructor->user,'LS'.$instructor->user->id.'T','Teacher','Congratulations! Your Teacher Account Has Been Unblocked Successfully!','Teacher Account Unblocked'));
                    break;
                case STATUS_REJECTED:
                    ...
                    Mail::to($instructor->user->email)->send(new UserBlockedMail($instructor->user,$instructor->status_message,'Teacher'));
                    break;
