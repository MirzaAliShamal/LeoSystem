...

{
    ...
    public function __construct(BlockingHistory $blockingHistory,Organization $organization){
        $this->blockingHistoryModel = $blockingHistory;
...

    {
        ...
        $data['title'] = __('Organizations Blocking History');

        $data['blocked_organizations'] = BlockingHistory::orderBy('id', 'DESC')->organizations()->paginate(25);

        return view('admin.organizations.blocking_history', $data);
...

    {
        ...
        $user->mobile_number = str_replace($request->area_code,'',$request->phone_number);

        $user->phone_number = str_replace($request->area_code,'',$request->phone_number);
        ...
        $organizationData = [
            ...
            'phone_number' => str_replace($request->area_code,'',$request->phone_number),
...

    {
        ...
        Mail::to($organization->user->email)->send(new UserRejectedMail($organization->user,$organization->status_message,'Organization'));
...

    {
        ...
        try {
            ...
            Mail::to($organization->user->email)->send(new UserApprovedMail($organization->user,'LS'.$organization->user->id.'OR','Organization'));
            $this->showToastrMessage('success', 'Organization status has been updated');

            return redirect()->back()->with(['success' => __('Organization status has been updated')]);
...

    {
        ...
        try {
            ...
            $organization->status_message = $request->status_message;
            ...
            switch ($request->status){
                case STATUS_APPROVED:
                    $this->blockingHistoryModel->create([
                        ...
                        'type'    => USER_ROLE_ORGANIZATION,
                        'status'  => STATUS_APPROVED
                    ]);
                    Mail::to($organization->user->email)->send(new UserApprovedMail($organization->user,'LS'.$organization->user->id.'OR','Organization','Congratulations! Your Organization Account Has Been Unblocked Successfully!','Organization Account Unblocked'));
                    break;
                case STATUS_REJECTED:
                    ...
                    Mail::to($organization->user->email)->send(new UserBlockedMail($organization->user,$organization->status_message,'Organization'));
                    break;
            }
