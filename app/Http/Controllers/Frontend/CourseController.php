...
class CourseController extends Controller
{
    ...
    public function allCourses(Request $request)
    {
        ...
        $initialCount = 4; // Initially loaded subcategories
        $allRemaining = $request->input('allRemaining', false);

        $data['categories'] = Category::with('subcategories')->active()->get();
        $limit = 4;

        $data['categories'] = Category::withCount(['courses' => function (Builder $query) {
            $query->where('private_mode', '!=', 1);
        }])->skip($offset)->take($limit)->get();


        $data['subcategories'] = Subcategory::withCount(['courses' => function (Builder $query) {
            $query->where('private_mode', '!=', 1);
        }])->skip($offset)->take($limit)->get();
        ...
        if ($allRemaining) {
            $data['subcategories'] = Subcategory::withCount(['courses' => function (Builder $query) {
                $query->where('private_mode', '!=', 1);
            }])->where('id', '>', $request->input('lastId'))->get();
        } else {
            $data['subcategories'] = Subcategory::withCount(['courses' => function (Builder $query) {
                $query->where('private_mode', '!=', 1);
            }])->skip($offset)
        ...
        $data['moreSubcategories'] = !$allRemaining;
        ...
        $data['popularCourses'] = $this->filterCourseData($request);
        $data['popularCourses'] = Course::where('private_mode', '!=', 1)->with('course_instructors.user.instructor.instructor_courses')->active()->paginate($this->coursePaginateValue);
        $organizationUsers = User::query()
            ...
            ->select('users.*', \DB::raw('NULL as organization_id'));  // Add NULL as organization_id

        $instructorUsers = User::query()
        ...
        // Combine both queries using union and apply limit
        $data['professionalTeachers'] = $organizationUsers
            ->union($instructorUsers)
            ->limit(12) // Apply limit to the final result
            ->get();
    ...
    public function courseDetails($slug)
    {
        ...
        $data['TotalScormCount'] = Course_lecture::where('course_id', $data['course']->id)
            ->count();
