...
class MainIndexController extends Controller
{
    public function index()
    {
        if (file_exists(storage_path('installed'))) {
            ...
            $data['TopBestTabs'] = ['Organization', 'Teachers'];
    ...
    public function userProfile($roles,User $user)
    {
    ...
    public function organizations(Request $request)
    {
        ...
        $priceMax = User::query()
            ...
            ->selectRaw('MAX(case when org.id is null then org.hourly_rate else org.hourly_rate end) AS hourly_rate')
            ->first();
    ...
    public function filterQuery($request)
    {
        ...
        if ($request->search) {
            $users->where(function ($q) use ($request) {
                $q->where('ins.first_name', 'LIKE','%'.$request->search.'%');
                $q->orWhere('ins.last_name', 'LIKE','%'.$request->search.'%');
                $q->orWhere('ins.id', 'LIKE','%'.$request->search.'%');
                $q->orWhere('org.id','LIKE', '%'.$request->search.'%');
                $q->orWhere('org.first_name','LIKE', '%'.$request->search.'%');
                $q->orWhere('org.last_name','LIKE', '%'.$request->search.'%');
            });
        }
        ...
        if (is_array($request->native_speaker)) {
            $users->whereHas('instructor', function($query) use ($request) {
               return $query->whereIn('native_speaker', $request->native_speaker);
            });
        }
    ...
    public function comingSoon()
    {
        ...
        return view('install.coming-soon', $data);
    }
